...
class Snackbar private constructor(context: Context) : SnackbarLayout(context) {
    ...
    private var actionColor = mUndefinedColor
    var isShowAnimated = true
        private set
    private var isDismissAnimated = true
    ...
    enum class SnackbarPosition(val layoutGravity: Int) {
     ...
     * @return
     */
    private fun lineColor(lineColor: Int?): Snackbar {
    ...
    fun init(
        ...
        if (mDrawable != mUndefinedDrawable) {
            ContextCompat.getDrawable(context, mDrawable)?.let { setBackgroundDrawable(layout, it) }
        }
        ...
        if (mCanSwipeToDismiss && res.getBoolean(R.bool.sb__is_swipeable)) {
            inner.setOnTouchListener(
                SwipeDismissTouchListener(this, "",
    ...
    fun updateWindowInsets(targetActivity: Activity?, outInsets: Rect) {
        outInsets.bottom = 0
    ...
    fun focusForAccessibility(view: View?) {
    ...
    fun isNavigationBarHidden(root: ViewGroup): Boolean {
    ...
    fun isNavigationBarTranslucent(targetActivity: Activity): Boolean {
    ...
    fun dismiss(animate: Boolean) {
        ...
        val slideOut =
            AnimationUtils.loadAnimation(context, getOutAnimationResource(mPhonePosition))
    ...
    protected fun updateLayoutParamsMargins(targetActivity: Activity?, params: MarginLayoutParams) {
        if (mUsePhoneLayout) {
            // Phone
